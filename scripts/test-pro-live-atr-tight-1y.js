...
for(let i=60;i<M.length-1;i++){
 ...
 if(R[i-1]>=C.buyLevel&&R[i]<C.buyLevel&&B==='BUY'&&h!==8&&!fomc&&ADX[i]>=20&&ar>=0.60&&ar<=1.25&&body>=0.50&&mom>=-35)side='BUY';
...
console.log('\nPRO LIVE + TIGHTER ATR TEST - 1 YEAR');
console.log('DATA:',file,'| M5:',M.length);
console.log('FILTERS: ADX TEST BUY20 SELL18 | ATR TEST BUY .60-1.25 SELL .75-1.30 | BODY TEST BUY .50 SELL .45 | BUY MOM3H >= -35 | BUY BLOCK08 | WED 17-20 UTC BLOCK');
