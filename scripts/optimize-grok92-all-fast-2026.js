...
const GC=[];
for(let i=260;i<M.length-25;i++){const b=M[i],entry=M[i+1]?.o;if(!Number.isFinite(entry)||b.t<FROM.getTime()||b.t>TO.getTime()||!(A[i]>0))continue;
 const up=E9[i-1]<=E21[i-1]&&E9[i]>E21[i],dn=E9[i-1]>=E21[i-1]&&E9[i]<E21[i];let side=up?'BUY':dn?'SELL':null;if(!side)continue;
 const va=M.slice(i-20,i).reduce((s,x)=>s+x.v,0)/20,h=before(H,b.t);if(!(va>0)||h<229||!(HA[h]>0))continue;
 const bias=HC[h]>H200[h]?'BUY':HC[h]<H200[h]?'SELL':null;if(side!==bias)continue;
 const meta={rsi:R[i],gap:Math.abs(E9[i]-E21[i])/A[i],vol:b.v/va,adx:HD[h],dist:Math.abs(HC[h]-H200[h])/HA[h],hour:new Date(b.t).getUTCHours()};
 const base=A[i]*1.5,risk=base*1.225,td=base*.8*(side==='BUY'?.90:.85),x=exitFixed(M,i,side,entry,side==='BUY'?entry-risk:entry+risk,side==='BUY'?entry+td:entry-td,24);x.meta=meta;push(GC,i,side,x,b.t);
}
function gf(s){return `T${s.t} WR${s.wr.toFixed(1)} PF${s.pf.toFixed(2)} N${s.n>=0?'+':''}${s.n.toFixed(2)}R DD${s.dd.toFixed(2)}`}
function gp(side,rBuy,rSell,gap,vol,adx,dist){return GC.filter(x=>x.side===side&&(side==='BUY'?x.meta.rsi>rBuy:x.meta.rsi<rSell)&&x.meta.gap>=gap&&x.meta.vol>=vol&&x.meta.adx>=adx&&x.meta.dist>=dist)}
const glo=FROM.getTime(),ghi=TO.getTime(),gmid=glo+Math.floor((ghi-glo)/2);
const grid=[];
