...
console.log('\n━━━━━━━━ SIDE-SPECIFIC EXIT OPTIMIZATION — RESEARCH ONLY ━━━━━━━━');
function replaySide(base,buyCfg,sellCfg){
 const out=[];
 for(const t of base){const c=t.side==='BUY'?buyCfg:sellCfg;out.push(...replayFixed([t],c.sm,c.tm))}
 return out.sort((a,b)=>a.exit-b.exit);
}
function temporalScore(z){
 const d180=ps(z,vt1-180*86400000,vt1+1),d90=ps(z,vt1-90*86400000,vt1+1);let pos=0,weak=0;
 for(let q=0;q<4;q++){const x=ps(z,vt0+vspan*q/4,q===3?vt1+1:vt0+vspan*(q+1)/4);if(x.net>0&&x.pf>1)pos++;if(x.net<=0||x.pf<1)weak++}
 return{d180,d90,pos,weak};
}
const jobs=[
 {k:'GROK',sms:[1.10,1.15,1.20,1.225,1.25,1.30],tms:[.80,.85,.90,.95,1.00]},
 {k:'RAPID',sms:[.90,1.00,1.05,1.10,1.15,1.20],tms:[.80,.85,.90,.95,1.00,1.05]},
];
for(const job of jobs){
 const b=BASE_TRADES[job.k],baseSt=stats(b),rows=[];
 for(const bsm of job.sms)for(const btm of job.tms)for(const ssm of job.sms)for(const stm of job.tms){
  const z=replaySide(b,{sm:bsm,tm:btm},{sm:ssm,tm:stm}),st=stats(z),tv=temporalScore(z);
  if(st.pf>=baseSt.pf*.95&&st.net>=baseSt.net*.90&&st.dd<=baseSt.dd*1.10&&tv.pos>=3&&tv.weak<=1)
   rows.push({bsm,btm,ssm,stm,st,...tv});
 }
 rows.sort((a,b)=>(b.st.wr-a.st.wr)||(b.st.pf-a.st.pf)||(b.st.net-a.st.net));
 console.log('\n'+defs[job.k].name+' | BASE '+f(baseSt));
 rows.slice(0,20).forEach((x,i)=>console.log(`${String(i+1).padStart(2,' ')} | BUY SLx${x.bsm} TPx${x.btm} | SELL SLx${x.ssm} TPx${x.stm} | FULL ${f(x.st)} | 180D ${f(x.d180)} | 90D ${f(x.d90)} | folds+${x.pos}/4 weak${x.weak}/4`));
}
console.log('\n━━━━━━━━ RANGE MR EXTENDED-DATA CHECK ━━━━━━━━');
console.log('Current local dataset entries: '+BASE_TRADES.RANGE.length+'. This section validates exit sensitivity only; it does NOT fabricate older signals.');
const rb=BASE_TRADES.RANGE,rrows=[];
for(let sm=.80;sm<=1.2001;sm+=.05)for(let tm=.75;tm<=1.1001;tm+=.05){const z=replayFixed(rb,sm,tm),st=stats(z),tv=temporalScore(z);rrows.push({sm:+sm.toFixed(2),tm:+tm.toFixed(2),st,...tv})}
rrows.sort((a,b)=>(b.st.pf-a.st.pf)||(b.st.wr-a.st.wr)||(b.st.net-a.st.net));
rrows.slice(0,15).forEach((x,i)=>console.log(`${String(i+1).padStart(2,' ')} | SLx${x.sm.toFixed(2)} TPx${x.tm.toFixed(2)} | FULL ${f(x.st)} | 180D ${f(x.d180)} | 90D ${f(x.d90)} | folds+${x.pos}/4 weak${x.weak}/4`));
console.log('\nSIDE-SPECIFIC RESEARCH COMPLETE — LIVE FILES UNCHANGED.');


console.log('\n━━━━━━━━ PORTFOLIO EXIT CANDIDATE SIMULATION — RESEARCH ONLY ━━━━━━━━');
const portBase=Object.values(BASE_TRADES).flat().sort((a,b)=>M[a.entryI].t-M[b.entryI].t);
const grokNew=replaySide(BASE_TRADES.GROK,{sm:1.225,tm:.90},{sm:1.225,tm:.85});
const rapidNew=replaySide(BASE_TRADES.RAPID,{sm:.90,tm:.90},{sm:1.05,tm:.85});
const portCand=Object.entries(BASE_TRADES).flatMap(([k,v])=>k==='GROK'?grokNew:k==='RAPID'?rapidNew:v).sort((a,b)=>M[a.entryI].t-M[b.entryI].t);
function portPeriod(z,a,b){return stats(z.filter(x=>{const t=M[x.entryI].t;return t>=a&&t<b}))}
function pp(lab,b,c){console.log(lab.padEnd(8)+' BASE '+f(b)+' | CAND '+f(c)+' | '+delta(b,c))}
pp('FULL',stats(portBase),stats(portCand));
pp('180D',portPeriod(portBase,vt1-180*86400000,vt1+1),portPeriod(portCand,vt1-180*86400000,vt1+1));
pp('90D',portPeriod(portBase,vt1-90*86400000,vt1+1),portPeriod(portCand,vt1-90*86400000,vt1+1));
for(let q=0;q<4;q++){const a=vt0+vspan*q/4,e=q===3?vt1+1:vt0+vspan*(q+1)/4;pp('F'+(q+1),portPeriod(portBase,a,e),portPeriod(portCand,a,e))}
console.log('\nMONTHLY BASE → CAND');
const months=[...new Set(portBase.concat(portCand).map(x=>new Date(M[x.entryI].t).toISOString().slice(0,7)))].sort();
for(const mo of months){const a=Date.parse(mo+'-01T00:00:00Z'),d=new Date(a),e=Date.UTC(d.getUTCFullYear(),d.getUTCMonth()+1,1);pp(mo,portPeriod(portBase,a,e),portPeriod(portCand,a,e))}
console.log('\nCHANGED STRATEGIES');
console.log('GROK BASE  '+f(stats(BASE_TRADES.GROK)));console.log('GROK CAND  '+f(stats(grokNew)));
console.log('RAPID BASE '+f(stats(BASE_TRADES.RAPID)));console.log('RAPID CAND '+f(stats(rapidNew)));
const pb=stats(portBase),pc=stats(portCand);
const pass=pc.wr>pb.wr&&pc.pf>=pb.pf&&pc.net>=pb.net&&pc.dd<=pb.dd;
console.log('\nPORTFOLIO VERDICT: '+(pass?'PASS':'HOLD')+' | requires exact sequential candidate rerun before any live change.');
console.log('PORTFOLIO RESEARCH ONLY — LIVE FILES UNCHANGED.');
