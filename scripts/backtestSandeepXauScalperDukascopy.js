...
const COST=.45;
const ema=(v,p)=>{let o=Array(v.length).fill(null),a=2/(p+1);if(!v.length)return o;o[0]=v[0];for(let i=1;i<v.length;i++)o[i]=a*v[i]+(1-a)*o[i-1];return o};
const roll=(v,p,fn)=>v.map((_,i)=>i<p-1?null:fn(v.slice(i-p+1,i+1)));
function rsi(v,p=7){let o=Array(v.length).fill(null);for(let i=p;i<v.length;i++){let g=0,l=0;for(let j=i-p+1;j<=i;j++){let d=v[j]-v[j-1];if(d>0)g+=d;else l-=d}o[i]=l?100-100/(1+g/l):100}return o}
function atr(c,p=14){let tr=c.map((x,i)=>i?Math.max(x.high-x.low,Math.abs(x.high-c[i-1].close),Math.abs(x.low-c[i-1].close)):x.high-x.low);return roll(tr,p,a=>a.reduce((s,x)=>s+x,0)/p)}
