...
(async()=>{
  console.log(`🏁 XAUUSD Backtest Tournament V2 | target=${HISTORY} 5m bars | dev=${Math.round(DEV_RATIO*100)}% | OOS=${Math.round((1-DEV_RATIO)*100)}%`);
  ...
  const [c5raw,c15raw,c1hraw]=await Promise.all([
    ...
    getGoldHistoricalCandles('15min',Math.min(40000,Math.ceil(HISTORY/3)+600)),
    getGoldHistoricalCandles('1h',Math.min(12000,Math.ceil(HISTORY/12)+300))
  ]);
  ...
  const jsonPath=path.join(OUT_DIR,'tournament-v2-latest.json');fs.writeFileSync(jsonPath,JSON.stringify(report,null,2));
  const csvPath=path.join(OUT_DIR,'tournament-v2-latest.csv');
