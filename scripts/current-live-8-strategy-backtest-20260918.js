...
console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');console.log('CURRENT LIVE 8-STRATEGY BACKTEST — 2026-09-18');console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');console.log(`DATA ${new Date(M[0].t).toISOString()} → ${new Date(M.at(-1).t).toISOString()} | M5=${M.length} H1=${H.length} D1=${D.length}`);console.log('Entry=next M5 open | one open trade per strategy | SL before TP on ambiguous bar\n');
for(const k of Object.keys(defs)){const a=trades[k];console.log(`\n${defs[k].name} | ${f(stats(a))}`);console.log(` BUY  ${f(stats(a.filter(x=>x.side==='BUY')))}`);console.log(` SELL ${f(stats(a.filter(x=>x.side==='SELL')))}`);const rs={};for(const x of a)rs[x.reason]=(rs[x.reason]||0)+1;console.log(' exits:',Object.entries(rs).map(([q,n])=>`${q}:${n}`).join(' | '));const mm={};for(const x of a)(mm[month(x.exit)]??=[]).push(x);for(const m of Object.keys(mm).sort())console.log(`  ${m} | ${f(stats(mm[m]))}`)}
const all=Object.values(trades).flat().sort((a,b)=>a.exit-b.exit);console.log('\n━━━━━━━━ PORTFOLIO SUM OF STRATEGY R ━━━━━━━━');console.log(f(stats(all)));const pm={};for(const x of all)(pm[month(x.exit)]??=[]).push(x);for(const m of Object.keys(pm).sort())console.log(`${m} | ${f(stats(pm[m]))}`);

// Cairo-time diagnostic. Egypt is UTC+3 throughout this 2025-09 -> 2026-09 sample.
function cairoMin(t){const d=new Date(t+3*3600000);return d.getUTCHours()*60+d.getUTCMinutes()}
function inWrap(m,a,b){return a<=b?(m>=a&&m<b):(m>=a||m<b)}
const windows=[
 ['03:30-04:00',210,240],
 ['05:00-06:00',300,360],
 ['22:00-00:00',1320,0],
...
console.log('\n━━━━━━━━ CAIRO ENTRY-TIME DIAGNOSTIC ━━━━━━━━');
console.log('Timezone: Africa/Cairo (UTC+3 for this dataset) | grouped by historical ENTRY time');
for(const [label,a,b] of windows){
 const z=all.filter(x=>inWrap(cairoMin(M[x.entryI].t),a,b));
 console.log(`\nCAIRO ${label} | PORTFOLIO ${f(stats(z))} | share ${(100*z.length/all.length).toFixed(1)}%`);
 for(const k of Object.keys(defs)){const q=trades[k].filter(x=>inWrap(cairoMin(M[x.entryI].t),a,b));if(q.length)console.log(`  ${defs[k].name.padEnd(24)} ${f(stats(q))}`)}
}
