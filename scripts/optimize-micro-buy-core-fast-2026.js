...
for(let i=260;i<M.length-20;i++){const b=M[i],hr=new Date(b.t).getUTCHours(),dow=new Date(b.t).getUTCDay(),entry=M[i+1]?.o;if(!Number.isFinite(entry)||b.t<FROM.getTime()||b.t>TO.getTime())continue;
 ...
 // Rapid V5 exact.
 if([0,1,3,4,5,7,8,9,10,11,13,14,15,16,17,21,23].includes(hr)&&AR[i]>0){const h=before(H,b.t);if(h>=52&&HAR[h]>0){const sep=Math.abs(H20[h]-H50[h])/HAR[h],bull=HC[h]>H20[h]&&H20[h]>H50[h]&&H20[h]>H20[h-2],bear=HC[h]<H20[h]&&H20[h]<H50[h]&&H20[h]<H20[h-2],hi=Math.max(...M.slice(i-3,i).map(x=>x.h)),lo=Math.min(...M.slice(i-3,i).map(x=>x.l)),rg=b.h-b.l,body=Math.abs(b.c-b.o),pos=rg?(b.c-b.l)/rg:.5;let side=bull&&b.c>hi&&pos>=.79&&b.c>E20[i]?'BUY':bear&&b.c<lo&&pos<=.15&&b.c<E20[i]?'SELL':null;if(side&&sep>=.06&&body/AR[i]>=.60&&rg/AR[i]<=2&&Math.abs(b.c-E20[i])/AR[i]<=1.2&&!(side==='BUY'&&[3,16,17].includes(hr))&&Math.abs(entry-b.c)<=AR[i]*.3){const swing=side==='BUY'?Math.min(b.l,M[i-1].l):Math.max(b.h,M[i-1].h),base=Math.max(AR[i]*.65,Math.abs(entry-swing));if(base<=AR[i]*1.35){const risk=base*(side==='BUY'?.90:1.05),td=base*(side==='BUY'?.85*.90:.8*.85);push(RA,i,side,exitFixed(M,i,side,entry,side==='BUY'?entry-risk:entry+risk,side==='BUY'?entry+td:entry-td,10),b.t)}}}
 // Grok92 exact.
...
const MC=[];
const SEP=[.10,.12,.15,.18,.20,.22,.25], IMP=[1.10,1.20,1.30,1.40,1.50,1.60,1.70], BODY=[.60,.65,.70,.75,.80,.85];
for(let i=260;i<M.length-20;i++){const b=M[i],hr=new Date(b.t).getUTCHours(),entry=M[i+1]?.o;if(!Number.isFinite(entry)||b.t<FROM.getTime()||b.t>TO.getTime()||[16,17,20,21].includes(hr)||!(A[i]>0))continue;
 const rg=b.h-b.l;if(!(rg>0)||!(E9A[i]>E21A[i]&&E21A[i]>E50A[i]&&E21A[i]>E21A[i-3]))continue;
 const is=i-4,ie=i-2,imp=M[ie].c-M[is].o;if(!(imp>0))continue;let pl=Infinity;for(let j=ie+1;j<i;j++)pl=Math.min(pl,M[j].l);
 const retr=(M[ie].c-pl)/imp,pos=(b.c-b.l)/rg;if(!(retr>=.12&&retr<=.35&&pos>=.78&&b.c>M[i-1].h))continue;
 const meta={sep:Math.abs(E9A[i]-E21A[i])/A[i],imp:imp/A[i],body:Math.abs(b.c-b.o)/rg};
 const risk=2*A[i],x=exitFixed(M,i,'BUY',entry,entry-risk,entry+risk,10);x.meta=meta;push(MC,i,'BUY',x,b.t);
}
function mf(s){return `T${s.t} WR${s.wr.toFixed(1)} PF${s.pf.toFixed(2)} N${s.n>=0?'+':''}${s.n.toFixed(2)}R DD${s.dd.toFixed(2)}`}
const live=MC.filter(x=>x.meta.sep>=.15&&x.meta.imp>=1.40&&x.meta.body>=.75);
console.log('\nMICRO BUY — CORE FAST OPTIMIZER (candidate replay; research only, live unchanged)');
console.log('BASE LIVE | '+mf(stat(live)));
const out=[];
for(const sep of SEP)for(const imp of IMP)for(const body of BODY){const a=MC.filter(x=>x.meta.sep>=sep&&x.meta.imp>=imp&&x.meta.body>=body);if(a.length<50)continue;const s=stat(a);out.push({sep,imp,body,...s})}
