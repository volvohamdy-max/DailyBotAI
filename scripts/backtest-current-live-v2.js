#!/usr/bin/env node
'use strict';
/** CURRENT LIVE V3 parity runner — E2-new + Rapid-V5 + G1 + P1 + Range-BUY + S0. */
...
rep("E2 + R3 + G1 + P1 + N4 + S0, MAX OPEN 2.","E2-new + Rapid-V5 + G1 + P1 + Range-BUY + S0, MAX OPEN 2.",'header');
// Promoted E2: BUY burst 2.2/wick .30, SELL burst 2.6/wick .25, retrace .15, TP .60 ATR.
rep("E2:{buyBurst:2.3,sellBurst:2.6,buyWick:.25,sellWick:.35,tp:.7}","E2:{buyBurst:2.2,sellBurst:2.6,buyWick:.30,sellWick:.25,tp:.6,retrace:.15}",'e2 config');
...
// E2 old backtest hard-coded retrace .20; make it read promoted .15.
...
rep("console.log('🧬 E2 + R3 + G1 + P1 + N4 + S0');","console.log('🧬 E2-new + Rapid-V5 + G1 + P1 + Range-BUY + S0');",'console combo');
...
s=s.replace("console.log('🔥 CURRENT LIVE EXACT BACKTEST');","console.log('🔥 CURRENT LIVE V3 BACKTEST — ALL PROMOTIONS');");
const generated=path.join(__dirname,'backtest-current-live-v3.generated.js');const m=new Module(generated,module);m.filename=generated;m.paths=Module._nodeModulePaths(__dirname);m._compile(s,generated);
