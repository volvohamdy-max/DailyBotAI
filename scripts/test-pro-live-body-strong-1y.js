...
for(let i=60;i<M.length-1;i++){
 ...
 if(R[i-1]>=C.buyLevel&&R[i]<C.buyLevel&&B==='BUY'&&h!==8&&!fomc&&ADX[i]>=17&&ar>=0.50&&ar<=1.30&&body>=0.50&&mom>=-35)side='BUY';
 else if(R[i-1]<=C.sellLevel&&R[i]>C.sellLevel&&B==='SELL'&&!fomc&&ADX[i]>=15&&ar>=0.70&&ar<=1.40&&body>=0.45)side='SELL';
...
console.log('\nPRO LIVE + STRONGER BODY TEST - 1 YEAR');
console.log('DATA:',file,'| M5:',M.length);
console.log('FILTERS: ADX BUY17 SELL15 | ATR BUY .50-1.30 SELL .70-1.40 | BODY TEST BUY .50 SELL .45 | BUY MOM3H >= -35 | BUY BLOCK08 | WED 17-20 UTC BLOCK');
