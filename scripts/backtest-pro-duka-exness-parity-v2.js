...
const exFile='data/xauusdm-m5-exness-2026.json';
let exRaw=JSON.parse(fs.readFileSync(exFile,'utf8'));if(!Array.isArray(exRaw))exRaw=exRaw.data||exRaw.candles||exRaw.rows||[];
const exTs=exRaw.map(x=>+(x.timestamp??x.time??x.t)).filter(Number.isFinite).sort((a,b)=>a-b);
if(!exTs.length)throw Error('No Exness timestamps');
const exStart=exTs[0],exEnd=exTs[exTs.length-1];
...
let M=raw.map(x=>({t:+(x.timestamp??x.time??x.t),o:+(x.open??x.o),h:+(x.high??x.h),l:+(x.low??x.l),c:+(x.close??x.c)})).filter(x=>[x.t,x.o,x.h,x.l,x.c].every(Number.isFinite)).sort((a,b)=>a.t-b.t);
const originalCount=M.length;
M=M.filter(x=>x.t>=exStart&&x.t<=exEnd);
console.log('EXNESS RANGE: '+new Date(exStart).toISOString()+' -> '+new Date(exEnd).toISOString()+' | M5='+exRaw.length);
console.log('DUKASCOPY SAME RANGE: M5='+M.length+' | original='+originalCount);
if(!M.length||M.length>=originalCount*0.9)throw Error('PARITY GUARD: Dukascopy range was NOT reduced');
...
console.log('TARGET OLD RESULT: T694 WR68.6 PF1.32 Net+65.09R DD9.45 LS7');
console.log('\nALL :',stats(trades));
