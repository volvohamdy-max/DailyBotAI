...
const C={rsiPeriod:14,buyLevel:37,sellLevel:63,buyExitLevel:55,sellExitLevel:45,stopDistance:12,cooldownMinutes:180,maxLossesPerDay:2,dailyEmaPeriod:50};
...
for(let i=60;i<M.length-1;i++){
 ...
 if(open){const sl=open.side==='BUY'?open.en-C.stopDistance:open.en+C.stopDistance;
 ...
 const B=bias.get(day);let side=null;
...
console.log('KEPT: RSI 37/63 | D1 EMA50 | SL12 | RSI EXIT 55/45 | CD180 | MAXLOSS2');
console.log('REMOVED: ADX | ATR | BODY | MOM3H | HOURS | SMART BLOCKS | FOMC');
console.log('\nALL :',stats(trades));
