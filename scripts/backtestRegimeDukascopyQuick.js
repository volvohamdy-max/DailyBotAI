...
source = source.replace(
  ...
  "⚡ GOLD REGIME — DUKASCOPY QUICK (4 VARIANTS)"
);
...
const replacement = `  const configs=[\n    {adxMin:23,tpR:1.5,${common}},\n    {adxMin:23,tpR:1.8,${common}},\n    {adxMin:26,tpR:1.5,${common}},\n    {adxMin:26,tpR:1.8,${common}}\n  ];`;

source = source.slice(0, start) + replacement + source.slice(end + endMarker.length);
source = source.replace('🔬 Variants=${configs.length}', '🔬 QUICK Variants=${configs.length}');

// Keep the original execution/statistics logic; only the tested grid is reduced to four configs.
eval(source);
