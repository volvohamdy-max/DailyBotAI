...
/* One-pass monthly + BUY/SELL ledger. Research only. */
...
const ledger=path.join(os.tmpdir(),`dailybot-six-ledger-${process.pid}.json`);
const tempEngine=path.join(__dirname,`.tmp-six-ledger-${process.pid}.js`);

/* Instrument a temporary copy only. Canonical engine and live code stay untouched. */
...
src=src.replace(marker,exportCode+marker);
fs.writeFileSync(tempEngine,src);
let r;
try{
 r=spawnSync(process.execPath,[tempEngine,FROM,TO],{cwd:root,stdio:'inherit',env:{...process.env,BACKTEST_LEDGER_JSON:ledger}});
} finally { try{fs.unlinkSync(tempEngine)}catch{} }
if(r?.error){console.error(r.error);process.exit(1)}
if(r?.status!==0)process.exit(r?.status||1);
if(!fs.existsSync(ledger)){console.error('❌ Ledger export missing.');process.exit(1)}
const data=JSON.parse(fs.readFileSync(ledger,'utf8'));try{fs.unlinkSync(ledger)}catch{}
...
console.log('\n🏆 PORTFOLIO BY SIDE');
for(const side of ['BUY','SELL'])console.log(`${side.padEnd(4)} ${fmt(stats(trades.filter(x=>x.side===side)))}`);
console.log(`\n✅ Ledger integrity: ${trades.length} accepted trades | engine=${data.acceptedCount} | MAX2 blocked=${data.rejectedCount}`);
console.log('🔒 Full year ran once. Live strategies/VIP routing untouched.');