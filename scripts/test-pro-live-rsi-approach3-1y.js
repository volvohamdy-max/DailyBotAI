...
for(let i=60;i<M.length-1;i++){
 ...
 if(R[i-3]>R[i-2]&&R[i-2]>R[i-1]&&R[i-1]>=C.buyLevel&&R[i]<C.buyLevel&&B==='BUY'&&h!==8&&!fomc&&ADX[i]>=20&&ar>=0.50&&ar<=1.30&&body>=0.55&&mom>=-25)side='BUY';
 else if(R[i-3]<R[i-2]&&R[i-2]<R[i-1]&&R[i-1]<=C.sellLevel&&R[i]>C.sellLevel&&B==='SELL'&&!fomc&&ADX[i]>=18&&ar>=0.75&&ar<=1.30&&body>=0.45)side='SELL';
...
console.log('\nPRO LIVE + RSI 3-BAR APPROACH TEST - 1 YEAR');
console.log('DATA:',file,'| M5:',M.length);
console.log('NEW TEST: RSI must approach threshold monotonically for 3 bars | EXISTING: ADX TEST BUY20 SELL18 | ATR BUY .50-1.30 SELL .75-1.30 | BODY TEST BUY .55 SELL .45 | BUY MOM3H TEST >= -25 | BUY BLOCK08 | WED 17-20 UTC BLOCK');
