#!/usr/bin/env node
'use strict';
// RESEARCH ONLY — validation of XAUUSD M5 rolling 15m breakout, BUY only, 19:00→02:00 UTC.
// Fixed $7 SL / $7 TP. Does NOT touch live strategies.
...
function inSession(h){return h>=19||h<2}
const trades=[];let nextAllowed=60;
for(let i=60;i<c.length-MAX_HOLD-1;i++){
 const h=new Date(c[i].t).getUTCHours();if(!inSession(h)||i<nextAllowed||!A[i]||!D[i]||!R[i]||A[i]>7||D[i]<19)continue;
 const b=c[i];if((b.h-b.l)>1.4*A[i])continue;
 let hi=-Infinity;for(let j=i-RANGE_BARS;j<i;j++)hi=Math.max(hi,c[j].h);
 if(!(b.c>hi&&E20[i]>E50[i]&&b.c>E20[i]&&R[i]>52))continue;
 const en=c[i+1].o,sl=en-7,tp=en+7;let r=0,bars=MAX_HOLD;
 for(let j=i+1;j<=i+MAX_HOLD;j++){const loss=c[j].l<=sl,win=c[j].h>=tp;if(loss||win){r=loss?-1:1;bars=j-i;break}}
 if(!r){const x=c[i+MAX_HOLD].c;r=Math.max(-1,Math.min(1,(x-en)/7))}
 trades.push({r,bars,t:c[i].t,hour:h,month:new Date(c[i].t).toISOString().slice(0,7)});nextAllowed=i+COOLDOWN_BARS;
}
function st(t){let net=0,g=0,l=0,w=0,pk=0,dd=0,ls=0,cur=0;for(const x of t){net+=x.r;pk=Math.max(pk,net);dd=Math.max(dd,pk-net);if(x.r>0){w++;g+=x.r;cur=0}else if(x.r<0){l-=x.r;cur++;ls=Math.max(ls,cur)}}return{n:t.length,wr:t.length?100*w/t.length:0,net,pf:l?g/l:(g?999:0),dd,ls}}
...
const dev=trades.filter(x=>x.t<cutT),hold=trades.filter(x=>x.t>=cutT);
console.log(`🧪 GOLD ROLLING 7x7 BUY 19→02 VALIDATION — RESEARCH ONLY | M5=${c.length}`);
console.log(`Period: ${new Date(c[0].t).toISOString()} -> ${new Date(c[c.length-1].t).toISOString()}`);
console.log('Rules: BUY only | UTC 19:00→02:00 | rolling 15m breakout | ADX>=19 | EMA20>EMA50 | RSI>52 | ATR<=7 | candle<=1.4ATR | SL=$7 TP=$7');
console.log('\n🌍 ALL  | '+fmt(st(trades)));
console.log('📚 DEV  | '+fmt(st(dev))+' | '+(dev[0]?new Date(dev[0].t).toISOString().slice(0,10):'-')+' → '+(dev.length?new Date(dev[dev.length-1].t).toISOString().slice(0,10):'-'));
console.log('🔐 HOLD | '+fmt(st(hold))+' | '+(hold[0]?new Date(hold[0].t).toISOString().slice(0,10):'-')+' → '+(hold.length?new Date(hold[hold.length-1].t).toISOString().slice(0,10):'-'));
console.log('\n📅 BY MONTH');
const months=[...new Set(trades.map(x=>x.month))];for(const m of months)console.log(`${m} | ${fmt(st(trades.filter(x=>x.month===m)))}`);
console.log('\n🕐 BY UTC HOUR');for(const h of [19,20,21,22,23,0,1])console.log(`${String(h).padStart(2,'0')}:00 | ${fmt(st(trades.filter(x=>x.hour===h)))}`);
