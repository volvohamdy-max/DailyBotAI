...
/* POST-QUALITY 7-STRATEGY PORTFOLIO BACKTEST — 2026-09-16
   ...
   This avoids silently rewriting Grok/Pro/Range/Sweep mechanics.
*/
...
// Exhaustion #408. Signal uses ex=i-1 as exhaustion candle and i as confirmation.
...
// Micro #1164: anchors are exact current baseline conditions.
must("if(hour(M[i].t)===20)return;const b=M[i],rg=b.h-b.l;if(!(rg>0)||Math.abs(b.c-b.o)/rg<.55||Math.abs(E9[i]-E21[i])/A[i]<.08)return;", "if([16,17,20].includes(hour(M[i].t)))return;const b=M[i],rg=b.h-b.l;if(!(rg>0)||Math.abs(b.c-b.o)/rg<.65||Math.abs(E9[i]-E21[i])/A[i]<.15)return;",'Micro #1164 filters');
must("if(pos<.66||b.c<=M[i-1].h)return;", "if(pos<.78||b.c<=M[i-1].h)return;",'Micro #1164 close position');
// Make report title unambiguous.
s=s.replace('FRESH LIVE PORTFOLIO BACKTEST — 2026-09-15','POST-QUALITY LIVE PORTFOLIO BACKTEST — 2026-09-16');
console.log('PATCH CHECK: Exhaustion #408 + Rapid #1297 + Micro #1164; Grok/Pro/Range/Sweep unchanged.');
