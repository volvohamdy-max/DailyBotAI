...
// D1 EMA50 bias from the full Dukascopy history; execution/indicators remain Exness M5.
const dukaFile='data/xauusd-m5-dukascopy-2025-09_to_2026-09.json';
let dr=JSON.parse(fs.readFileSync(dukaFile,'utf8'));if(!Array.isArray(dr))dr=dr.data||dr.candles||dr.rows||[];
const DM=dr.map(x=>({t:+(x.timestamp??x.time??x.t),c:+(x.close??x.c)})).filter(x=>Number.isFinite(x.t)&&Number.isFinite(x.c)).sort((a,b)=>a.t-b.t);
const days=new Map();for(const x of DM){let k=new Date(x.t).toISOString().slice(0,10);if(!days.has(k))days.set(k,{t:x.t,c:x.c});else days.get(k).c=x.c}
