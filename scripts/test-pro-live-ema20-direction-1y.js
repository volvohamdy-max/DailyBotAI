...
const R=rsi(M.map(x=>x.c),C.rsiPeriod),ATR=atr(M,14),ADX=adx(M,14),E20=ema(M.map(x=>x.c),20);
...
for(let i=60;i<M.length-1;i++){
 ...
 if(R[i-1]>=C.buyLevel&&R[i]<C.buyLevel&&B==='BUY'&&h!==8&&!fomc&&ADX[i]>=17&&ar>=0.50&&ar<=1.30&&body>=0.40&&mom>=-35&&b.c>E20[i])side='BUY';
 else if(R[i-1]<=C.sellLevel&&R[i]>C.sellLevel&&B==='SELL'&&!fomc&&ADX[i]>=15&&ar>=0.70&&ar<=1.40&&body>=0.35&&b.c<E20[i])side='SELL';
...
console.log('\nPRO LIVE + EMA20 DIRECTION TEST - 1 YEAR');
