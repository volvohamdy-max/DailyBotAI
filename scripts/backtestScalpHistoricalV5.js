...
function macdSeries(v,fast=12,slow=26,sig=9){const ef=ema(v,fast),es=ema(v,slow),m=Array(v.length).fill(null);for(let i=0;i<v.length;i++)if(ef[i]!=null&&es[i]!=null)m[i]=ef[i]-es[i];const vals=m.map(x=>x==null?0:x),sg=ema(vals,sig);return {m,s:sg}}
...
function ind(c){const close=c.map(x=>x.close);const md=macdSeries(close);return {e9:ema(close,9),e20:ema(close,20),e21:ema(close,21),e50:ema(close,50),e200:ema(close,200),r7:rsi(close,7),r14:rsi(close,14),a:atr(c,14),x:adx(c,14),v:sessionVwap(c),macd:md.m,macdSig:md.s}}
...
(async()=>{
 console.log(`🏁 Historical + Modern Scalping Tournament V5 | XAUUSD | 5m target=${HISTORY}`);
 ...
 // ==========================================================
 // HISTORICAL BOT STRATEGIES — reconstructed from backups
 // AI-dependent historical gates are intentionally omitted and tagged TECH_RECONSTRUCTION.
 // ==========================================================

 // 8) LEGACY GOLD SCALPER STRICT
 add('LEGACY_STRICT','🧱 Legacy Gold Scalper STRICT',{source:'backup',mode:'STRICT',reconstruction:'TECH'},i=>{
   const h=htf(i),a=i5.a[i],e9=i5.e9[i],e20=i5.e20[i],rr=i5.r14[i]; if(!h||!a||e9==null||e20==null||rr==null||!h.t15)return null;
   const m=mom(c5,i,3),side=h.t15; if(!m||m.side!==side)return null;
   const emaOk=side==='BUY'?e9>e20:e9<e20; if(!emaOk)return null;
   if(!(side==='BUY'?rr>=50&&rr<=70:rr<=50&&rr>=30))return null;
   const xx=i5.x[i]; if(xx!=null&&xx<20)return null;
   const vv=i5.v[i]; if(vv!=null && !(side==='BUY'?c5[i].close>=vv:c5[i].close<=vv))return null;
   return {side,atr:a,lookback:12,buf:.12,minRiskAtr:.5,tag:'LEGACY_STRICT_TECH'};
 });

 // 9) LEGACY GOLD SCALPER EARLY
 add('LEGACY_EARLY','🌅 Legacy Gold Scalper EARLY',{source:'backup',mode:'EARLY',reconstruction:'TECH'},i=>{
   const h=htf(i),a=i5.a[i],e20=i5.e20[i],rr=i5.r14[i]; if(!h||!a||e20==null||rr==null||!h.t15)return null;
   const side=h.t15,m=mom(c5,i,3); const price=c5[i].close;
   if(side==='BUY'){if(!(price>e20&&rr>=50&&rr<=67))return null;if(m?.side==='SELL'&&m.strength>=3)return null;}
   else {if(!(price<e20&&rr<=50&&rr>=33))return null;if(m?.side==='BUY'&&m.strength>=3)return null;}
   if(Math.abs(price-e20)/a>.85)return null;
   return {side,atr:a,lookback:10,buf:.12,minRiskAtr:.5,tag:'LEGACY_EARLY_TECH'};
 });

 // 10) LEGACY GOLD SCALPER BREAKOUT
 add('LEGACY_BREAKOUT','💥 Legacy Gold Scalper BREAKOUT',{source:'backup',mode:'BREAKOUT',reconstruction:'TECH'},i=>{
   const h=htf(i),a=i5.a[i],e9=i5.e9[i],e20=i5.e20[i],rr=i5.r14[i]; if(!h||!a||e9==null||e20==null||rr==null||!h.t15||i<7)return null;
   const side=h.t15,m=mom(c5,i,3),rg=recentRange(c5,i,6),p=c5[i].close; if(!m||m.side!==side||!rg)return null;
   if(side==='BUY'){if(!(p>rg.hi&&(p-rg.hi)<=a*.55&&e9>e20&&rr>=53&&rr<=78))return null;}
   else {if(!(p<rg.lo&&(rg.lo-p)<=a*.55&&e9<e20&&rr<=47&&rr>=22))return null;}
   return {side,atr:a,lookback:8,buf:.10,minRiskAtr:.5,tag:'LEGACY_BREAKOUT_TECH'};
 });

 // 11) AGGRESSIVE BREAKOUT-A backup variant
 add('AGGRESSIVE_BREAKOUT_A','🔥 Aggressive BREAKOUT-A',{source:'backup',reconstruction:'TECH'},i=>{
   const h=htf(i),a=i5.a[i],e9=i5.e9[i],e20=i5.e20[i],rr=i5.r14[i]; if(!h||!a||e9==null||e20==null||rr==null||!h.t15||i<7)return null;
   const side=h.t15,m=mom(c5,i,3),rg=recentRange(c5,i,6),p=c5[i].close;if(!m||m.side!==side||m.strength<2||!rg)return null;
   if(side==='BUY'){if(!(p>rg.hi&&(p-rg.hi)<=a*.45&&e9>e20&&rr>=55&&rr<=78))return null;}
   else {if(!(p<rg.lo&&(rg.lo-p)<=a*.45&&e9<e20&&rr<=45&&rr>=22))return null;}
   return {side,atr:a,lookback:8,buf:.10,minRiskAtr:.45,tag:'AGGRESSIVE_BREAKOUT_A'};
 });

 // 12) V2 PULLBACK / RECLAIM historical reconstruction
 for(const adxMin of [24,28]) add('V2_PULLBACK','🔁 Gold Scalper V2 Pullback',{adxMin,source:'backup'},i=>{
   const h=htf(i),a=i5.a[i],e9=i5.e9[i],e20=i5.e20[i],rr=i5.r14[i],xx=i5.x[i],vv=i5.v[i]; if(!h||!a||e9==null||e20==null||rr==null||xx==null||vv==null||!h.t15||xx<adxMin)return null;
   const side=h.t15,m=mom(c5,i,3); if(!m||m.side!==side)return null;
   if(!(side==='BUY'?e9>e20:e9<e20))return null;
   if(!(side==='BUY'?c5[i].close>vv:c5[i].close<vv))return null;
   const rec=c5.slice(Math.max(0,i-3),i+1),tol=.15*a;
   const touch=rec.some(x=>x.low<=e20+tol&&x.high>=e20-tol)||rec.some(x=>x.low<=vv+tol&&x.high>=vv-tol); if(!touch)return null;
   if(!candle(c5,i,side)||bodyAtr(c5,i,i5.a)>.55)return null;
   if(!(side==='BUY'?rr>=48&&rr<=68:rr<=52&&rr>=32))return null;
   if(Math.abs(c5[i].close-vv)/a>.65)return null;
   return {side,atr:a,lookback:12,buf:.15,minRiskAtr:.55,tag:'V2_PULLBACK'};
 });

 // 13) GOLD POWER historical strategy (15M + 1H, 5 of 6 confirmations)
 add('GOLD_POWER','🏆 Gold Power',{source:'backup',timeframe:'15M/1H'},i=>{
   const t=c5[i].time+FIVE,j15=closedIndex(c15,t,FIFTEEN),j1=closedIndex(c1,t,HOUR); if(j15<60||j1<60)return null;
   const a=i15.a[j15],e20_15=i15.e20[j15],e50_15=i15.e50[j15],e20_1=i1.e20[j1],e50_1=i1.e50[j1],rr=i15.r14[j15],xx=i15.x[j15],vv=i15.v[j15];
   if([a,e20_15,e50_15,e20_1,e50_1,rr,xx,vv].some(x=>x==null))return null;
   const mm=(j15>=3)?(c15[j15].close-c15[j15-3].close):0,price=c5[i].close;
   const b=[e20_15>e50_15,e20_1>e50_1,rr>=52&&rr<=70,xx>=28,price>vv,mm>0];
   const s=[e20_15<e50_15,e20_1<e50_1,rr<=48&&rr>=30,xx>=28,price<vv,mm<0];
   let side=null;if(b[0]&&b[1]&&b[3]&&b.filter(Boolean).length>=5)side='BUY';else if(s[0]&&s[1]&&s[3]&&s.filter(Boolean).length>=5)side='SELL';if(!side)return null;
   return {side,atr:a,lookback:16,buf:.15,minRiskAtr:1.5,tag:'GOLD_POWER'};
 },[2,3]);

 // 14) TREND HUNTER technical reconstruction (AI gate unavailable historically)
 for(const scoreMin of [65,78]) add('TREND_HUNTER','🧭 Trend Hunter TECH',{scoreMin,source:'backup',reconstruction:'TECH'},i=>{
   const t=c5[i].time+FIVE,j=closedIndex(c15,t,FIFTEEN);if(j<60)return null;
   const e20=i15.e20[j],e50=i15.e50[j],mm=i15.macd[j],ms=i15.macdSig[j],rr=i15.r14[j],xx=i15.x[j],a=i15.a[j];if([e20,e50,mm,ms,rr,xx,a].some(x=>x==null))return null;
   const side=e20>e50&&mm>ms?'BUY':e20<e50&&mm<ms?'SELL':null;if(!side||xx<20)return null;
   let sc=45; if(xx>=35)sc+=25;else if(xx>=30)sc+=22;else if(xx>=25)sc+=18;else sc+=10;
   const rReady=side==='BUY'?rr>=52&&rr<=66:rr>=34&&rr<=48; if(rReady)sc+=15;else if(side==='BUY'?rr>50&&rr<70:rr>30&&rr<50)sc+=10;
   if(sc<scoreMin)return null; if(side==='BUY'?rr>=68:rr<=32)return null;
   return {side,atr:a,lookback:14,buf:.12,minRiskAtr:.8,tag:'TREND_HUNTER_TECH'};
 });

 // 15) LEGACY SCALPING ENTRY ENGINE EMA9/21 + RSI + room-to-target
 add('LEGACY_ENTRY_ENGINE','🧰 Legacy Scalp Entry Engine',{source:'backup'},i=>{
   const h=htf(i),a=i5.a[i],e9=i5.e9[i],e21=i5.e21[i],rr=i5.r14[i];if(!h||!h.t15||!a||e9==null||e21==null||rr==null)return null;
   const side=h.t15,trigger=side==='BUY'?(e9>e21&&rr>=50&&rr<=72):(e9<e21&&rr<=50&&rr>=28);if(!trigger)return null;
   const m=mom(c5,i,3);if(!m||m.side!==side)return null;
   if(Math.abs(c5[i].close-e9)>a*.85)return null;
   const rg=recentRange(c5,i,18);if(!rg)return null;const obstacle=side==='BUY'?rg.hi:rg.lo;if(Math.abs(obstacle-c5[i].close)<a*.60)return null;
   const atrPct=a/c5[i].close*100;if(atrPct>.16||atrPct<.02)return null;
   return {side,atr:a,lookback:12,buf:.10,minRiskAtr:.5,tag:'LEGACY_ENTRY_ENGINE'};
 });
 ...
 for(const [strategy,arr] of Object.entries(byStrategy)){
   ...
   const qualified=final.trades>=8 && final.netR>0 && final.pf>=1.20 && final.exp>0 && final.dd<=12;
 ...
 const report={generatedAt:new Date().toISOString(),history:{m5:c5.length,m15:c15.length,h1:c1.length},costs:{spread:SPREAD,slippagePerSide:SLIP},split:'60/20/20',qualification:{minFinalTrades:8,minPF:1.20,minNetR:0,minExpectancy:0,maxDD:12},ranking:selected.map(x=>({...x,fn:undefined})),top3:qualifiers.map(x=>({strategy:x.strategy,label:x.label,params:x.params,tpR:x.tpR,final:x.final}))};
 const jp=path.join(OUT,'scalp-historical-v5-latest.json');fs.writeFileSync(jp,JSON.stringify(report,null,2));
 const cp=path.join(OUT,'scalp-historical-v5-latest.csv');fs.writeFileSync(cp,'rank,strategy,label,qualified,tp,trades,wr,net_r,pf,exp,dd,mfe,mae\n'+selected.map((x,i)=>[i+1,x.strategy,JSON.stringify(x.label),x.qualified,x.tpR,x.final.trades,x.final.winRate.toFixed(2),x.final.netR.toFixed(2),x.final.pf.toFixed(3),x.final.exp.toFixed(3),x.final.dd.toFixed(2),x.final.mfe.toFixed(3),x.final.mae.toFixed(3)].join(',')).join('\n')+'\n');
