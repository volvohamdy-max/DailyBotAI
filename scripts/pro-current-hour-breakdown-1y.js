...
function done(side,en,ex,t){let pnl=side==='BUY'?ex-en:en-ex,r=pnl/(side==='BUY'?C.buyStop:C.sellStop);trades.push({side,r,h:new Date(t).getUTCHours()});if(r<0){let d=new Date(t).toISOString().slice(0,10);loss[d]=(loss[d]||0)+1;cool=t+C.cooldownMinutes*60000}open=null}
for(let i=60;i<M.length-1;i++){
 ...
 if(R[i-1]>=C.buyLevel&&R[i]<C.buyLevel&&B==='BUY'&&h!==8&&!fomc&&ADX[i]>=17&&ar>=0.50&&ar<=1.30&&body>=0.40&&mom>=-35)side='BUY';
 else if(R[i-1]<=C.sellLevel&&R[i]>C.sellLevel&&B==='SELL'&&!fomc&&ADX[i]>=15&&ar>=0.70&&ar<=1.40&&body>=0.35)side='SELL';
...
console.log('\nPRO 1106 + FOUND FILTERS - 1 YEAR');
console.log('DATA:',file,'| M5:',M.length);
console.log('FILTERS: ADX BUY17 SELL15 | ATR BUY .50-1.30 SELL .70-1.40 | BODY BUY .40 SELL .35 | BUY MOM3H >= -35 | BUY BLOCK08 | WED 17-20 UTC BLOCK');
...
console.log('UTC HOUR BREAKDOWN');
for(let h=0;h<24;h++){const a=trades.filter(x=>x.h===h);if(a.length)console.log(String(h).padStart(2,'0')+':00',stats(a))}
console.log('');
