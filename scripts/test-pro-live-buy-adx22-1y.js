...
for(let i=60;i<M.length-1;i++){
 ...
 if(R[i-1]>=C.buyLevel&&R[i]<C.buyLevel&&B==='BUY'&&h!==8&&!fomc&&ADX[i]>=22&&ar>=0.50&&ar<=1.30&&body>=0.50&&mom>=-25)side='BUY';
...
console.log('\nPRO LIVE + BUY ADX22 TEST - 1 YEAR');
console.log('DATA:',file,'| M5:',M.length);
console.log('FILTERS: ADX TEST BUY22 SELL18 | ATR BUY .50-1.30 SELL .75-1.30 | BODY TEST BUY .50 SELL .45 | BUY MOM3H TEST >= -25 | BUY BLOCK08 | WED 17-20 UTC BLOCK');
