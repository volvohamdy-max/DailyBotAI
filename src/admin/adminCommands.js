...
`;
    ...
    // إشارات التداول اليدوية مخصصة للـ VIP فقط.
    // لا نستخدم الجروب العام ولا broadcast للمستخدمين هنا.
    if (!config.vipChannelId) {
        return ctx.reply(
            '❌ VIP_CHANNEL_ID غير مضبوط — لم يتم إرسال الإشارة لأي مكان'
        );
    }
    ...
    try {
        ...
        console.log("✅ Signal sent to VIP channel");

    } catch(e){
        console.log("VIP signal error:", e.message);
        return ctx.reply(
            `❌ فشل إرسال الإشارة للـ VIP
${e.message}`
        );
    }
    ...
    // تأكيد للأدمن
    return ctx.reply(
        '✅ تم إرسال الإشارة لقناة VIP فقط'
    );
