...
async function scan(){
 ...
 if(side==='BUY'&&(ADX[i]<17||atrRatio<0.50||atrRatio>1.30||bodyRatio<0.40||momentum3h===null||momentum3h< -35))return wait('PRO_BUY_FILTER_BLOCK',{adx5:ADX[i],atrRatio,bodyRatio,momentum3h});
 if(side==='SELL'&&(ADX[i]<15||atrRatio<0.70||atrRatio>1.40||bodyRatio<0.35))return wait('PRO_SELL_FILTER_BLOCK',{adx5:ADX[i],atrRatio,bodyRatio});
