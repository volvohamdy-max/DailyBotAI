...
const CONFIG = {
  ...
  buyStopMult: 1.225, sellStopMult: 1.225, buyTpMult: 0.90, sellTpMult: 0.85,
  rewardR: 0.8, buyAdxMin: 20, sellAdxMin: 22, rsiBuyMin: 52, rsiSellMax: 48,
  buyEmaGapAtr: 0.02, sellEmaGapAtr: 0.02, buyVolumeSpikeMult: 1.25, sellVolumeSpikeMult: 1.00,
  buyH1DistanceAtr: 0.40, sellH1DistanceAtr: 0.15, sessionsUTC: [[0, 24]]
};
...
async function scanGrokGold92Strategy(){
 ...
 const emaGapAtr=Math.abs(e9[i]-e21[i])/atr5,requiredEmaGap=side==='BUY'?CONFIG.buyEmaGapAtr:CONFIG.sellEmaGapAtr;if(emaGapAtr<requiredEmaGap)return wait('GROK92_EMA_GAP_TOO_SMALL',{side,rsi5,atr5,emaGapAtr,requiredEmaGap});
 const volumes=c5.map(x=>finite(x.volume)??0),volAvg=sma(volumes.slice(0,-1),CONFIG.volumePeriod),signalVolume=volumes.at(-1),requiredVolumeSpike=side==='BUY'?CONFIG.buyVolumeSpikeMult:CONFIG.sellVolumeSpikeMult;if(!(volAvg>0)||!(signalVolume>=volAvg*requiredVolumeSpike))return wait('GROK92_VOLUME_NOT_SPIKE',{side,rsi5,atr5,signalVolume,volumeAverage:volAvg,volumeRatio:volAvg>0?signalVolume/volAvg:null,requiredVolumeSpike});
 const hc=closes(c1h),h1Ema200=ema(hc,200),ha=atrSeries(c1h,CONFIG.atrPeriod),hd=adxSeries(c1h,CONFIG.atrPeriod),h=c1h.length-1,h1CloseNow=finite(c1h[h].close),h1Atr=ha[h],h1Adx=hd[h];if(![h1CloseNow,h1Ema200,h1Atr,h1Adx].every(Number.isFinite)||!(h1Atr>0))return wait('GROK92_H1_INDICATORS_NOT_READY',{side,rsi5,atr5});const requiredAdx=side==='BUY'?CONFIG.buyAdxMin:CONFIG.sellAdxMin;if(h1Adx<requiredAdx)return wait('GROK92_H1_ADX_TOO_WEAK',{side,rsi5,atr5,h1Adx,requiredAdx});
 const h1Bias=h1CloseNow>h1Ema200?'BUY':h1CloseNow<h1Ema200?'SELL':null;if(side!==h1Bias)return wait('GROK92_H1_BIAS_MISMATCH',{side,h1Bias,rsi5,atr5,h1Adx});const h1Distance=Math.abs(h1CloseNow-h1Ema200)/h1Atr,requiredH1Distance=side==='BUY'?CONFIG.buyH1DistanceAtr:CONFIG.sellH1DistanceAtr;if(h1Distance<requiredH1Distance)return wait('GROK92_H1_TOO_CLOSE_EMA200',{side,h1Bias,h1Adx,h1Distance,requiredH1Distance});
 const signalBar=c5[i],signalKey=barKey(signalBar);if(signalKey&&STATE.lastSentSignalBar===signalKey)return wait('GROK92_SIGNAL_ALREADY_SENT',{side,signalKey});const entry=finite(livePrice)??finite(raw5?.at(-1)?.close)??finite(signalBar.close);if(!Number.isFinite(entry))return wait('GROK92_NO_LIVE_PRICE',{side});const baseRisk=atr5*CONFIG.stopAtr,stopMult=side==='BUY'?CONFIG.buyStopMult:CONFIG.sellStopMult,tpMult=side==='BUY'?CONFIG.buyTpMult:CONFIG.sellTpMult,risk=baseRisk*stopMult,tpDistance=baseRisk*CONFIG.rewardR*tpMult,stopLoss=side==='BUY'?entry-risk:entry+risk,target=side==='BUY'?entry+tpDistance:entry-tpDistance;STATE.pendingSignalBar=signalKey||null;
 const score=Math.min(100,Math.round(72+Math.min(10,Math.max(0,(h1Adx-requiredAdx)*.8))+Math.min(8,emaGapAtr*40)+Math.min(10,Math.max(0,(signalVolume/volAvg-requiredVolumeSpike)*12))));
 return{ready:true,status:'GROK92_READY',pair,direction:side,strategyId:CONFIG.id,strategyLabel:CONFIG.label,entryMode:'EMA_CROSS_VOLUME_H1_REGIME_Q101',grade:'A',score,aiConfidence:0,entry,stopLoss,tp1:target,tp2:target,risk,rrTp1:tpDistance/risk,rrTp2:tpDistance/risk,atr5,rsi5,adx5:null,adx15:null,h1Adx,h1Bias,h1Ema200,h1Atr,h1Distance,ema9:e9[i],ema21:e21[i],emaGapAtr,signalVolume,volumeAverage:volAvg,volumeRatio:signalVolume/volAvg,signalBar:signalKey,reasons:['5M EMA9/21 fresh cross',`RSI ${side==='BUY'?'>':'<'} ${side==='BUY'?CONFIG.rsiBuyMin:CONFIG.rsiSellMax}`,`5M volume >= ${requiredVolumeSpike}x average`,`EMA gap >= ${requiredEmaGap} ATR`,`H1 ADX >= ${requiredAdx}`,'H1 price aligned with EMA200',`H1 distance >= ${requiredH1Distance} ATR`,'All-day scan enabled','Unified live-gold candles',`Side-specific exit: SL x${stopMult} / TP distance x${tpMult}`]};
}
