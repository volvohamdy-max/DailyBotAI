...
async function getBinanceGoldProxy(interval, wanted) {
  ...
  let rows = (Array.isArray(data) ? data : []).map(r => ({ timestamp:Number(r[0]), open:Number(r[1]), high:Number(r[2]), low:Number(r[3]), close:Number(r[4]), volume:Number(r[5] || 0) })).filter(r => Number.isFinite(r.timestamp) && [r.open,r.high,r.low,r.close].every(Number.isFinite));
  ...
  rows = rows.map(c => ({ ...c, open:c.open*calibration, high:c.high*calibration, low:c.low*calibration, close:c.close*calibration }));
...
async function getGoldCandlesResilient(interval, wantedBars = null) {
  ...
  if (cached && cached.candles?.length >= required && cacheIsCurrent(cached, interval)) {
    console.log(`GOLD CANDLE RECOVERY CACHE: XAUUSD:${interval} | source=Binance-PAXG-Proxy | bars=${cached.candles.length}`);
    return cached.candles;
  }
  if (inFlight.has(key)) { console.log(`Shared gold proxy request: XAUUSD:${interval}`); return inFlight.get(key); }
  const promise = (async () => { const proxy = await getBinanceGoldProxy(interval, required); recoveryCache.set(key, { candles:proxy, time:Date.now(), source:'Binance-PAXG-Proxy' }); return proxy; })();
