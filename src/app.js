...
const registerTradingCommandCenter = require('./commands/tradingCommandCenter');
...
function removeLegacyGoldPowerData() {
  try {
    ...
    try { const deletePerformance = db.prepare('DELETE FROM trade_performance WHERE trade_id = ?'); for (const trade of powerTrades) deletePerformance.run(Number(trade.id)); }
...
async function main() {
  ...
  if (!config.botToken) { console.error('BOT_TOKEN is required. Copy .env.example to .env and configure it.'); process.exit(1); }
  await db.ready; initDatabase(); removeLegacyGoldPowerData(); console.log('Database is ready.');
  const bot = new Telegraf(config.botToken); installTelegramMarketHashtags(bot);
  ...
  try { registerVirtualPortfolio(bot); await bot.launch(); console.log('Telegram Forex AI bot is running. Open Telegram and send /start.'); }
  catch (error) { console.error('Failed to launch Telegram bot. Check BOT_TOKEN and internet connection.'); console.error(error.message); process.exit(1); }
